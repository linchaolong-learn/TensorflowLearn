...
def inference(input_tensor, train, regularizer):
    ...
    # 通过使用不同的命名空间来隔离不同层的变量，这可以让每一层中变量命名只需要
    # 考虑在当前层的作用，而不需要担心重名问题。和标准LeNet-5模型不大一样，这里
    # 定义的卷积层输入为28x28x1的原始MNIST图像像素。因为卷积层中使用了全0填充，
    # 所以输出为28x28x32的矩阵
    ...
    # 实现第四次池化层的前向传播过程。这一层和第二层的结构是一样的。这一层的输入为
